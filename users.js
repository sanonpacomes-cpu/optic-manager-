  ...
  try {
    ...
    if (req.method === 'GET') {
      const users = await sql`
        SELECT id, full_name, login_id, email, phone, role, agency_name, is_active, must_change_password, login_count, last_login_at, created_at, updated_at
        FROM users_app
    ...
    if (req.method === 'POST') {
      ...
      if (!b.full_name || !loginId || !b.password || !b.role) {
        return res.status(400).json({ error:'Nom, identifiant, mot de passe et rôle obligatoires' });
      }
      if (!isValidRole(b.role)) return res.status(400).json({ error:'Rôle invalide' });

      const rows = await sql`
        INSERT INTO users_app (full_name, login_id, email, phone, password_hash, role, agency_name, is_active)
      ...
      if (b.password) {
        const rows = await sql`
          UPDATE users_app SET
            ...
            email = ${b.email === undefined ? null : b.email || null},
            phone = ${b.phone === undefined ? null : b.phone || null},
            role = COALESCE(${b.role || null}, role),
            agency_name = ${b.agency_name === undefined ? null : b.agency_name || null},
